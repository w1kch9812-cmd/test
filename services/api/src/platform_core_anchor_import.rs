...
mod tests {
    ...
    fn rejects_wrong_entry_srid() {
        let err = parse_anchor_entry(&VALID_ENTRY.replace("EPSG:4326", "EPSG:3857"))
            .expect_err("wrong srid");
    ...
    fn rejects_invalid_checksum_shape() {
        let err =
            parse_anchor_entry(&VALID_ENTRY.replace(&"b".repeat(64), "not-a-sha256-checksum"))
    ...
    fn parses_anchor_jsonl_rows_and_rejects_row_count_mismatch() {
        ...
        let err = parse_anchor_rows(&format!("{VALID_ENTRY}\n"), 2)
            .expect_err("anchor row count mismatch");
