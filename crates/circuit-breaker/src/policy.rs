...
//!
//! Gongzzang-owned adapters use named policies at their owning boundary.
...
impl Policy {
    /// Platform Core internal API policy.
    ///
    /// - timeout 5 seconds / retry once with short backoff
    /// - 5 failures in 10 seconds opens for 30 seconds
    #[must_use]
    pub const fn platform_core_default() -> Self {
        Self {
            timeout_ms: 5_000,
            max_retries: 1,
            retry_base_ms: 500,
...
mod tests {
    ...
    #[test]
    fn platform_core_default_matches_internal_service_contract() {
        let p = Policy::platform_core_default();
        assert_eq!(p.timeout_ms, 5_000);
        assert_eq!(p.max_retries, 1);
        assert_eq!(p.retry_base_ms, 500);
