...
pub async fn truncate_all(pool: &PgPool) {
    sqlx::query(
        r#"truncate "user", listing, listing_photo, audit_log, outbox_event, admin_action, business_verification_queue, listing_review_queue, listing_report, featured_content, system_alert, pipeline_run, pipeline_schedule, bookmark_listing, bookmark_external, search_history, analysis_report, notification cascade"#,
    )
    .execute(pool)
    .await
