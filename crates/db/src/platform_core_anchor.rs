...
pub enum PlatformCoreEventInboxStatus {
    ...
    PendingImport,
    /// Event import is currently running.
    Processing,
    /// Event import finished successfully.
    Processed,
    /// Event import failed and needs inspection or retry.
    Failed,
}
...
/// Load the stored event payload for an inbound Platform Core event.
///
/// # Errors
///
/// Returns [`RepoError::Database`] when Postgres rejects the read.
pub async fn find_inbox_event_payload(
    pool: &PgPool,
    event_id: &str,
...
/// Load pending anchor import event ids for batch importer workers.
///
/// Includes `processing` rows so an importer process that exited after claiming
/// an event can be retried. Concurrent duplicate imports are still prevented by
/// the importer advisory lock.
///
/// # Errors
///
/// Returns [`RepoError::Database`] when Postgres rejects the read.
pub async fn find_pending_anchor_import_event_ids(
    pool: &PgPool,
    limit: i64,
    ...
    sqlx::query_scalar(
        ...
        where event_type = 'catalog.parcel_marker_anchor.snapshot.published.v1'
          ...
          and status in ('pending_import', 'processing')
...
/// Mark a pending or previously claimed Platform Core inbox event as processing.
///
/// Returns true when this call claimed the event, false when the event was
/// already processed, failed, or accepted.
///
/// # Errors
///
/// Returns [`RepoError::Database`] when Postgres rejects the update.
pub async fn mark_inbox_event_processing(pool: &PgPool, event_id: &str) -> Result<bool, RepoError> {
    let result = sqlx::query(
        r"
        update platform_core_event_inbox
        set status = $2,
            processed_at = null,
            failed_at = null,
            failure_reason = null
    ...
    .bind(event_id)
    .bind(PlatformCoreEventInboxStatus::Processing.as_str())
    .execute(pool)
    .await
    .map_err(map_sqlx_err)?;
...
/// Mark a processing Platform Core inbox event as processed.
///
/// # Errors
///
/// Returns [`RepoError::Database`] when the event does not exist, is not in the
/// processing state, or Postgres rejects the update.
pub async fn mark_inbox_event_processed(pool: &PgPool, event_id: &str) -> Result<(), RepoError> {
    let result = sqlx::query(
        r"
        update platform_core_event_inbox
        set status = $2,
    ...
    .bind(event_id)
    .bind(PlatformCoreEventInboxStatus::Processed.as_str())
    .execute(pool)
    .await
    .map_err(map_sqlx_err)?;
    ...
    Err(RepoError::Database(format!(
        "platform core event inbox row is not processing: event_id={event_id}"
    )))
}

/// Mark a processing Platform Core inbox event as failed.
///
/// # Errors
///
/// Returns [`RepoError::Database`] when the event does not exist, is not in the
/// processing state, or Postgres rejects the update.
pub async fn mark_inbox_event_failed(
    pool: &PgPool,
    event_id: &str,
    reason: &str,
) -> Result<(), RepoError> {
    ...
    .bind(event_id)
    .bind(PlatformCoreEventInboxStatus::Failed.as_str())
    .bind(reason)
    .execute(pool)
    .await
    .map_err(map_sqlx_err)?;
...
#[expect(
    ...
    reason = "anchor import staging, anchor upsert, and projection refresh stay in one transaction"
)]
pub async fn import_anchor_rows(
    ...
    let projection_result = sqlx::query(
        r"
    ...
    Ok(PlatformCoreAnchorImportReport {
        ...
        refreshed_listing_projection_count: projection_result.rows_affected(),
    })
}
