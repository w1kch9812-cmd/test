...
| Latest Gongzzang implementation commits | `ceff952` internal route SSOT guardrail, `eaae1ac` user-facing i18n guardrail, `ebb0133` home smoke page removal and app-entry redirect, `0d6e364` production DB-health debug route denial, `07bb4f9` local script fake-pass guardrail hardening, `14f9269` production dev-X9 route denial, `c3495b8` workflow `true` fallback guardrail, `9cc1399` workflow fake-pass fallback guardrail, `18883bb` production-shaped frontend build environment, `674ac08` workflow-file forbidden marker scan, `e6a8802` CI forbidden marker workflow gate, `59f5a4c` mojibake implementation marker guardrail, `eca931d` forbidden implementation marker guardrail, `9a4ae24` production repeated secret rejection, `fb7f072` production Redis TLS URL fail-fast, `9be311a` production public HTTPS URL fail-fast, `f317f64` production session secret sentinel rejection, `ae9b109` production platform-core base URL fail-fast, `7f1646d` production internal auth secret fail-fast, `738f06c` Naver Maps public client ID fail-fast, `550744e` api-types generation fail-fast, `8946709` listing photo signed download routing, `7964dc3` listing photo upload confirmation lifecycle hardening, `8c0e002` listing photo R2 config isolation |
...
| User-facing i18n guardrail | `eaae1ac` adds a TSX AST guardrail for Korean text literals across web production surfaces and UI primitives, then moves the detected strings into i18n messages or caller-provided labels | Covered locally |
| Internal route SSOT guardrail | `ceff952` adds a TS/TSX AST guardrail that rejects internal route string literals outside `lib/routes.ts`, then moves listing/auth/profile/proxy/event references to `ROUTES`, `API`, and `AUTH_PATH_PREFIX` | Covered locally |
...
| Local and workflow fake-pass prevention | `d224a88` removes tool-missing echo fallbacks from `lefthook.yml`; `9cc1399` extends `scripts/lefthook/check-no-fake-pass.{sh,tests.sh}` to scan GitHub workflow YAML files and rejects shell echo fallbacks; `c3495b8` rejects workflow `true` fallbacks and replaces the remaining workflow uses with explicit file/process existence checks; `07bb4f9` extends the guardrail to top-level `scripts/*.sh` and rewrites `setup-dev-tippecanoe.sh` to use explicit success/failure branches instead of `\|\| true` fallback masking | Covered locally |
| Dev-only map diagnostic route production denial | `14f9269` moves `/dev-x9-test` interactive map diagnostics behind a server page plus proxy-level production 404, keeps the client-only map probe in `dev-x9-test-client.tsx`, and adds unit plus live production HTTP smoke evidence; `07bb4f9` also removes `/dev-x9-test` from the platform-core event receiver cache invalidation target | Covered locally |
...
# ok - rejects script true fallback
...
# 1 test passed; RED before 0d6e364 was missing public_router/tower
...
pnpm --filter @gongzzang/web test -- tests/unit/home-page.test.tsx tests/unit/proxy.test.ts tests/unit/user-facing-i18n.test.ts tests/unit/internal-route-ssot.test.ts
...
# 36 web test files passed, 159 tests passed, 1 skipped; api-types contract test passed

bash -n scripts/setup-dev-tippecanoe.sh
# shell syntax check passed

pnpm --filter @gongzzang/web build
# production-shaped env build passed

pnpm --dir apps/web exec next start -p 3901; curl.exe -s -o NUL -w "%{http_code}" http://127.0.0.1:3901/dev-x9-test
# 404; temporary server process stopped after smoke (historical 14f9269 evidence)
...
bash scripts/lefthook/check-forbidden-implementation-markers.tests.sh
# ok - allows explicit names and words containing attempts
...
cargo check -p etl-base-layer
cargo test -p etl-base-layer
cargo clippy -p etl-base-layer --all-targets -- -D warnings
# etl-base-layer check passed; tests: 78 unit + 2 integration passed; clippy passed
