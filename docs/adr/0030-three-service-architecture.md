...
**gongzzang + dawneer + platform-core** 세 서비스로 이루어진 **γ' Three-Service Architecture**를 채택한다.
`gongzzang3`, `seanal-sms`, `Seanal Site Management System` 은 legacy repo path 또는
historical reference 로만 사용하고, 신규 설계 식별자는 lowercase slug 를 따른다.

| 서비스 | 도메인 | 사용자 | 소유 데이터 |
|---|---|---|---|
| **platform-core** | 내부 허브, 외부 노출 없음 | `gongzzang`, `dawneer` (서비스 consumer) | Catalog Context (IndustrialComplex, Parcel, Building, Manufacturer, Blueprint, SpatialLayer, DigitalTwin3d, IndustryGroup) · Workforce/Authz Context (Staff, Role, Session) · 공통 Outbox 버스 |
| **gongzzang** | **gongzzang.com (B2C 부동산 플랫폼)** | 일반 소비자 (Zitadel consumer org) + 별도 공짱 어드민 | Listing, ListingPhoto, RealTransaction, CourtAuction, BookmarkListing, SearchHistory, AnalysisReport |
| **dawneer** (`Dawneer`) | **B2B 산단 관리·사이트 제작 workbench** | 산단 관리 직원 (Zitadel staff org via platform-core) | Site, SitePage, SiteDeployment, SiteCampaign, KakaoNotificationLog, site-specific presentation override |
...
**Cross-domain SSO**: `gongzzang` 은 B2C 제품과 별도 공짱 어드민을 가진다.
`dawneer` 는 제품 자체가 Staff 기반 admin/workbench 이다. 두 admin surface 는
platform-core Workforce/Authz Context 를 통해 **Zitadel staff organization 기반 단일 로그인**을 공유한다.

## 컨텍스트

### 1. 문제 배경

gongzzang 과 dawneer 는 독립적으로 발전해 왔으나, 2026-04 기준 다음 중복 문제가 누적됐다.

- **신원/권한 로직 이중화**: gongzzang 은 Zitadel JWT 검증을 자체 보유(`crates/auth`),
  dawneer 는 Better Auth + 자체 `team` / `permission` / `rolePermission` 테이블을 보유한다.
  동일 직원이 두 시스템에서 서로 다른 역할을 가질 수 있어 권한 일관성이 깨진다.
- **산단 마스터 데이터 이중화**: gongzzang 의 `crates/domain/core/industrial-complex` 와
  `crates/domain/core/parcel` 가 V-World/data.go.kr 에서 ETL 한 사실 데이터를 소유하는데,
  dawneer 는 별도 `industrialComplex` / `parcelInfo` 테이블에 부분 사본을 유지한다.
  동기화 누락이 발생하면 Dawneer workbench 가 outdated 정보를 표시한다.
...
| 옵션 | 설명 | 결함 |
|---|---|---|
| A. 모놀리스 통합 | gongzzang + dawneer 를 단일 코드베이스로 병합 | 배포 독립성 0, 팀 자율성 소멸 |
...
- platform-core 를 독립 서비스로 운영하면 **배포 독립성**과 **공유 도메인 로직의 단일 소스(SSOT)**
  를 동시에 달성한다. ETL 파이프라인 변경이 gongzzang 과 dawneer 양쪽에 즉시 반영된다.
...
**채택 불가**. gongzzang 의 Rust ETL 파이프라인과 dawneer 의 Next.js 스택은
...
gongzzang 과 dawneer 는 platform-core API 를 통해서만 이 컨텍스트에 접근한다.
...
```
                ...
                ┌──────────────────┴────┐   ┌─────┴────────────────────┐
                │      gongzzang       │   │       dawneer         │
                │  gongzzang.com (B2C) │   │  Dawneer workbench    │
                │                       │   │                          │
...
3. **인증 위임**: gongzzang admin / dawneer 는 Zitadel ID Token 검증을 platform-core
   ...
   gongzzang 의 B2C 일반 사용자는 별도 Zitadel consumer organization 으로 격리되고
   gongzzang 이 직접 검증한다 (Workforce Context 와 무관).
4. **Cross-domain SSO**: Zitadel staff organization 의 session cookie 가
   `gongzzang` admin 과 `dawneer` workbench 의 OIDC redirect 후 발급되어 한 번 로그인이
   다른 admin surface 로 자동 이어진다. 2FA 는 Zitadel 측 정책에 위임.
