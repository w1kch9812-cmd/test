...
2. **OQ-2 — quota gate post-cutover.** The `request_cap`/daily-budget gate currently lives in `select_pending_jobs`. On Kafka it must move into a *consumer-side* rate limiter (not partition count). Confirm that re-homing before any cutover.
...
5. **OQ-5 — R2 content hash source.** Does the R2 writer expose a trustworthy per-object ETag/sha, or must the worker tee-hash the upload stream? (Determines whether the integrity block is free or needs a streaming hasher.)
6. **OQ-6 — boundary publication.** Keep the fabric Platform-Core-private and publish only the `collection.raw_written` event-type names + schema as the gongzzang/dawneer consumer contract (recommended), vs. share schemas in `shared-kernel`?
