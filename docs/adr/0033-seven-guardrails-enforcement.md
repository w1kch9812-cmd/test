...
- **G1 후보**: dawneer 의 `industrial-group.repository.ts` 가 Drizzle `db.execute<T>()`
  ...
  소유하면 dawneer 가 직접 SQL 을 쓰지 않고 typed API 만 호출하는 패턴으로 강제 (G1 + G2 결합).
- **G4 후보**: 현재 dawneer `event_outbox` 와 gongzzang outbox-event 가 별개 스키마를
  사용하고 `schemaVersion` 필드가 명시적이지 않다. platform-core 진입 시 공통
  `event-schemas/` 로 통합하며 schema registry 검증 도입.
- **G6 후보**: dawneer 의 `parcelInfo` 테이블이 gongzzang 의 `parcel` 마스터 데이터를
...
```typescript
// ❌ 위반: dawneer 가 platform-core 가 소유할 Catalog 테이블을 직접 조회
...
- **Rust 측**: `crates/api-types` (gongzzang 과 platform-core 가 공유)
- **TS 측**: `packages/api-contracts/` Zod 스키마 (dawneer 가 소비)
...
카카오 알림톡, Cloudflare R2 (도면·이미지 스토리지, S3-compatible API). 모두 도메인 계층은 trait/interface 만
...
# gongzzang CODEOWNERS
/crates/domain/core/listing/    @team-marketplace
/crates/data-pipeline-control/  @team-data
/apps/web/                       @team-frontend

# dawneer CODEOWNERS
/dawneer/apps/web/src/components/blueprint/  @team-blueprint
/dawneer/packages/core/                       @team-backend
```
