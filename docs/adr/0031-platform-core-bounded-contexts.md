...
|---|---|---|
| **Catalog Context** | 산업단지 마스터 데이터 (산단·필지·건물·제조사) 의 소유 및 외부 데이터 소스(V-World, data.go.kr) ETL | IndustrialComplex, Parcel, Building, Manufacturer, ParcelKind, PNU |
...
> (`User`, `BookmarkListing`, `SearchHistory` 등) 는 gongzzang3 가 직접 소유하며,
...
[Catalog Context] ----ACL----> [Workforce/Authz Context]
   ...
   integrates with:                    integrates with:
     gongzzang3 (Consumer)              gongzzang3 admin (Consumer)
     seanal-sms (Consumer)              seanal-sms (Consumer)
     V-World ETL                        Zitadel OIDC (Upstream)
     data.go.kr ETL
```

관계 유형: **Customer-Supplier** (Catalog/Workforce 가 Supplier, gongzzang3·seanal-sms 가 Customer)
...
- gongzzang3 는 `industrial-complex` / `parcel` 엔티티를 자체 소유하지만, seanal-sms 도
...
platform-core 는 Rust 기반 (ADR 0001 기준 gongzzang3 와 동일 스택) Cargo workspace 로 구현한다.

```
platform-core/
  crates/
    catalog/
      catalog-domain/         # Pure domain: IndustrialComplex, Parcel, Building, Manufacturer
        src/
    ...
    shared-kernel/             # 컨텍스트 공통 ValueObject + 이벤트 contract
      src/
        ...
        pnu.rs                 (PNU 검증 — gongzzang3 의 shared-kernel 과 동일 규칙)
        events/
          catalog_v1.rs        (IndustrialComplexCreated, ParcelKindChanged, ...)
          workforce_v1.rs      (StaffRoleAssigned, StaffSessionRevoked, ...)
```

이 구조는 gongzzang3 의 기존 `crates/domain/core/{industrial-complex,parcel,building,manufacturer}`
...
| PNU | 19자리 부동산 고유번호 — gongzzang3 shared-kernel 과 동일 검증 규칙 (ADR 0018) |
...
**용어 충돌 주의**: gongzzang3 에는 B2C 일반 사용자를 가리키는 `User` 엔티티가 있다.
