...
원자적으로 변경하는 것이 불가능해졌다.

- seanal-sms 에서 새 사이트를 생성하면, platform-core Catalog 에 사이트 항목이 등록되어야 한다.
- gongzzang3 에서 지도 레이어가 업데이트되면, seanal-sms 의 사이트 빌더가 최신 데이터를 반영해야 한다.
- platform-core 에서 사용자 역할이 변경되면, gongzzang3 와 seanal-sms 의 권한 캐시가 무효화되어야 한다.
...
| 데이터 종류 | 허용 지연 | 채택 기둥 |
|---|---|---|
| 상품/서비스 카탈로그 (읽기) | 최대 5분 | Read-Through Cache |
| 역할/권한 (읽기) | 최대 60초 | Read-Through Cache |
| 도메인 이벤트 (사이트 생성, 레이어 업데이트) | 수 초 이내 | Transactional Outbox |
| 결제 완료 확인 | 0초 (즉시) | Write-Through |
| 인증 토큰 검증 | 0초 (즉시) | Write-Through |
| 사용자 UI 인터랙션 | 즉시 응답 (충돌 허용) | Optimistic UI |
...
- platform-core 장애 시 gongzzang3 / seanal-sms 기능 전체 중단
...
**대상**: Catalog 상품/서비스 목록, Workforce 역할/권한 목록

```
Consumer (gongzzang3 / seanal-sms)
  |
...
- 캐시 키: `{context}:{entityType}:{id}:{version}` (버전 포함으로 stale 탐지)
...
**대상**: 도메인 이벤트 (사이트 생성, 레이어 업데이트, 멤버 변경 등)
...
**대상**: 결제 완료, 인증 토큰 발급, 중요 권한 변경

```
Client --> seanal-sms (요청)
             |
             +--> platform-core (동기 API 호출, 타임아웃 3초)
             |         |
             |    응답 수신 (성공/실패)
             |
             +--> 로컬 DB 업데이트 (platform-core 응답 기반)
             |
             +--> Client 응답
```
...
**대상**: 사용자 인터랙션 (포스트 저장, 팀원 추가, 사이트 설정 변경 등)
...
```typescript
// seanal-sms 포스트 저장 훅
function useOptimisticPostSave(post: Post) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (updated: Post) => savePost(updated),
    ...
    onError: (_err, updated, context) => {
      queryClient.setQueryData(['post', updated.id], context?.previous);
...
|---|---|---|---|
| 카탈로그 조회 | Cache | 5분 | 비가시적 |
| 역할 변경 후 접근 | Cache + Outbox | 60초 | 비가시적 |
| 결제 완료 | Write-Through | 0초 | 즉각 반영 |
| 포스트 저장 | Optimistic UI | 즉각 (충돌 시 롤백) | 즉각 응답 |
| 사이트 생성 이벤트 | Outbox | 수 초 | 비가시적 |
...
- Outbox 이벤트 P99 처리 지연이 30초를 초과하는 경우 (브로커 도입 검토)
- Cache invalidation 버그로 인한 데이터 불일치 인시던트 발생 시 (TTL 전략 재검토)
- Optimistic conflict rate 가 5% 초과하는 경우 (낙관적 잠금 전략 재검토)
