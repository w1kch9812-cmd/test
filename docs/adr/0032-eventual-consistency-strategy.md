...
- **catalog ETL → consumer 캐시**: platform-core 가 V-World/data.go.kr 에서 산단 마스터
  ...
  saenal.com 의 도면 에디터가 변경된 사실을 반영해야 한다.
- **parcel kind 변경 → 산단 운영**: platform-core 에서 한 필지의 `kind` 가
  `공장용지` → `지원시설` 로 갱신되면, seanal-sms 의 `ParcelIndustryGroup` 매핑이 영향을
  받고, gongzzang3 의 listing 검색 인덱스도 재계산되어야 한다.
- **Staff 역할 변경 → 권한 캐시**: platform-core Workforce 에서 직원 역할이 변경되면,
  gongzzang3 admin 과 seanal-sms 의 로컬 role cache 가 60초 이내에 무효화되어야 한다.
- **B2B 산단 컨텐츠 → B2C 노출**: seanal-sms 에서 운영자가 `Site` (산단 공식 사이트) 의
  메타데이터를 발행하면, gongzzang3 의 산단 상세 페이지에 해당 링크가 표시되어야 한다.
...
- seanal-sms: `SitePublished`, `SiteDeploymentCompleted` (gongzzang3 가 산단 상세에서 노출)
- gongzzang3: `ListingPublished`, `RealTransactionIngested` (관리·분석 용도)
...
```
Client --> seanal-sms (요청: 신규 산단 등록)
             |
             +--> platform-core /catalog/complexes (동기 API, 타임아웃 3초)
             |         |
             |    응답: 201 + ComplexId (성공) | 409 (중복 PNU) | 5xx (실패)
             |
             +--> seanal-sms 로컬 DB 의 InteractiveBlueprint 레코드를 ComplexId 로 연결
             |
             +--> Client 응답
```
...
```typescript
// seanal-sms IndustryGroup 트리 노드 이름 편집 — Optimistic UI + 409 충돌 처리
...
| Site 발행 → gongzzang3 노출 | Outbox | 수 초 | 비가시적 |
